...
class Insumo(models.Model):
    ...
    def save(self, *args, **kwargs):
        # Si NO es compuesto, calculamos costo en base al mercado y merma
    ...
    def calcular_costo_desde_subreceta(self):
        """
        Recorre los ingredientes hijos, suma sus costos y actualiza el costo de este insumo padre.
        Se asume que la receta está definida para producir 1 UNIDAD de este insumo.
        """
        if not self.es_insumo_compuesto:
            return
            
        total_costo = Decimal('0.0')
        # Usamos los ingredientes definidos en IngredienteCompuesto
        for componente in self.componentes.all():
            # Costo = Cantidad necesaria * Costo unitario del hijo
            costo_linea = componente.cantidad * componente.insumo_hijo.costo_unitario
            total_costo += costo_linea
            
        self.costo_unitario = total_costo
        # Guardamos sin disparar señales recursivas complejas, o usamos update
        Insumo.objects.filter(pk=self.pk).update(costo_unitario=total_costo)
